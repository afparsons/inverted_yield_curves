...
class YieldCurve_Monthly:
            ...
            if diff_tup[1] < 0:
                for j in range(i+1, len(diffed)-1):
                    if diffed[j] > diffed[j-1]:
                        inversion_periods.append(InversionPeriod_Monthly("date", (diffed[i][0], self.yields[diffed[i][0]]), (diffed[j][0], self.yields[diffed[j][0]])))
                        break
    ...
    #                         inversion_periods.append(InversionPeriod_Monthly("date", (0, self.yields[0]), (j-1, self.yields[j-1])))
    #                         break
    #         except TypeError:
    #             if isinstance(self.yields[0], type(None)):
    #                 pass
    ...
    #                             if self.yields[j] > self.yields[j-1]:
    #                                 minima_indices.append(j-1)
    #                                 inversion_periods.append(InversionPeriod_Monthly("date", (i, self.yields[i]), (j-1, self.yields[j-1])))
    #                                 break
    #                 except TypeError:
    #                     if isinstance(self.yields[i], type(None)):
    #                         continue
    #                     elif self.yields[i] >= self.yields[i-1] and i+1 == length-1:
    #                         maxima_indices.append(i)
    #                         inversion_periods.append(InversionPeriod_Monthly("date", (i, self.yields[i]), (i, self.yields[i])))
    #                     elif isinstance(self.yields[i-1], type(None)):
    #                         if self.yields[i] >= self.yields[i-2] and self.yields[i] > self.yields[i+1]:
    #                             maxima_indices.append(i)
    #                             for j in range(i+1, length-1):
    #                                 # print("<" + str(j) + ", " + str(self.yields[j]) + ">")
    #                                 if self.yields[j] > self.yields[j-1]:
